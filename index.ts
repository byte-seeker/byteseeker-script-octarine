...
new (class AutoBootsUtility {
	...
	private PostDataUpdate(delta: number): void {
		...
		if (this.treadsEnabled.value && this.switchQueue.length > 0) {
			...
			const powerTreads = hero.Items.find((item: Item) => item.Name === "item_power_treads") as
				| item_power_treads
				| undefined
		...
		if (this.phaseEnabled.value && !this.phaseSleeper.Sleeping) {
			...
			if (hero.Buffs.some((buff: any) => buff.Name === "modifier_item_phase_boots_active")) {
				return
			}
			...
			const phaseBoots = hero.Items.find((item: Item) => item.Name === "item_phase_boots")
			if (phaseBoots) {
				const ready =
