...
new (class RubickCombo {
 private readonly entry = Menu.AddEntry("Byteseeker").AddNode("Rubick Combo");

 private readonly comboKey = this.entry.AddKeybind(
  "Combo Key",
  "F",
  "Hold to execute Rubick combo",
 );
 private readonly comboRadius = this.entry.AddSlider(
 ...
 private readonly autoStealNode = this.entry.AddNode(
  "Auto Steal Spells (Background)",
 );
 private readonly autoStealEnabled = this.autoStealNode.AddToggle(
  "Enabled",
  true,
 );
 private autoStealGrid: any;

 private readonly autoStealHudNode =
  this.autoStealNode.AddNode("Floating HUD Panel");
 private readonly autoStealHudEnabled = this.autoStealHudNode.AddToggle(
  "Show HUD Panel",
  true,
 );
 ...
 private readonly autoStealHudX = this.autoStealHudNode.AddSlider(
  "HUD Position X",
  400,
  0,
  2500,
 );
 private readonly autoStealHudY = this.autoStealHudNode.AddSlider(
  "HUD Position Y",
  200,
  0,
  2500,
 );
 private readonly autoStealHudIconSize = this.autoStealHudNode.AddSlider(
  "HUD Icon Size",
  36,
  20,
  80,
 ...
 private readonly hudOffsetNode = this.entry.AddNode(
  "HUD Adjustments (Resolution Fix)",
 );
 private readonly hudOffsetX = this.hudOffsetNode.AddSlider(
  "X Offset",
  0,
  -100,
  100,
 );
 private readonly hudOffsetY = this.hudOffsetNode.AddSlider(
  "Y Offset",
  0,
  -100,
  100,
 );
 private readonly hudSizeW = this.hudOffsetNode.AddSlider(
  "Width Offset",
  0,
  -50,
  50,
 );
 private readonly hudSizeH = this.hudOffsetNode.AddSlider(
  "Height Offset",
  0,
  -50,
  50,
 ...
 constructor() {
  this.autoStealGrid = this.autoStealNode.AddDynamicImageSelector(
   "Spells",
   [],
   new Map(),
  );
  this.autoCastGrid = this.autoCastNode.AddDynamicImageSelector(
   "Spells",
   [],
   new Map(),
  );
  ...
  EventsSDK.on(
   ...
   this.AbilityCooldownChanged.bind(this),
  );
 ...
 private IsAbilityVisibleOnHUD(abil: Ability | undefined): abil is Ability {
  return (
   abil !== undefined && abil.IsValid && !abil.IsHidden && abil.ShouldBeDrawable
  );
 }
  ...
  for (const stolenSpell of stolenSpells) {
   if (
    ...
    if (this.autoCastGrid.IsEnabled(stolenSpell.Name)) {
     const isTarget = stolenSpell.HasBehavior(
      DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET,
     );
     const isPosition = stolenSpell.HasBehavior(
      DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_POINT,
     );
     const isNoTarget = stolenSpell.HasBehavior(
     ...
     if (isNoTarget && hero.Distance2D(bestTarget) <= castRange) {
      ...
      this.sleeper.Sleep(
       GameState.InputLag * 1000 + stolenSpell.CastPoint * 1000 + 100,
      );
      return true;
 ...
 private OnDraw(): void {
    ...
    if (this.IsAbilityVisibleOnHUD(abil)) {
     if (
      ...
      if (visibleIndex < hud.AbilitiesRects.length) {
       ...
       const bgSize = new Vector2(
        textSize.x + bgPaddingX * 2,
        textSize.y + bgPaddingY * 2,
       );
  ...
  if (this.autoStealHudEnabled.value) {
   ...
   const panelWidth = Math.max(
    150,
    Math.min(N > 0 ? N : cols, cols) * (iconSize + gap) + gap,
   );
   ...
   const titleTextSize = RendererSDK.GetTextSize(
    titleText,
    fontName,
    fontSize,
    fontWeight,
   ...
   if (N === 0) {
    ...
    const noSpellsSize = RendererSDK.GetTextSize(
     noSpellsText,
     fontName,
     10,
     400,
    ...
    const nsY =
     panelY + headerHeight + (panelHeight - headerHeight - noSpellsSize.y) / 2;
    RendererSDK.Text(
     noSpellsText,
     new Vector2(nsX, nsY),
     Color.Gray,
    ...
    for (let i = 0; i < N; i++) {
     ...
     const prioText = `${i + 1}`;
     const prioSize = RendererSDK.GetTextSize(
      prioText,
      fontName,
      9,
      800,
     ...
     const badgeSize = new Vector2(
      prioSize.x + badgePaddingX * 2,
      prioSize.y + badgePaddingY * 2,
     );
  ...
  if (this.autoStealHudEnabled.value && this.dragSpellName !== undefined) {
   ...
   const dragIconPos = cursorPos.Subtract(
 ...
 private isValidSpell(abil: Ability | undefined): abil is Ability {
  ...
  const isPassive = abil.HasBehavior(
   DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_PASSIVE,
  );
  const isNoTarget = abil.HasBehavior(
   DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_NO_TARGET,
  );
  const isTarget = abil.HasBehavior(
   DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET,
  );
  const isPosition = abil.HasBehavior(
   DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_POINT,
  );
  const isToggle = abil.HasBehavior(
   DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_TOGGLE,
  );
  const isAutoCast = abil.HasBehavior(
   ...
   if (
    ...
    for (const abil of hero.Spells) {
     if (this.isValidSpell(abil)) {
      ...
      if (
       currentSpellPriority !== -1 &&
       newSpellPriority >= currentSpellPriority
      ) {
       shouldSteal = false;
       break;
      }
    ...
    if (!this.stealSleeper.Sleeping || isUpgradingPriority) {
     ...
     if (hero.Distance2D(owner) <= castRange + 400) {
      ...
      this.stealSleeper.Sleep(
       Math.max(
        800,
        GameState.InputLag * 1000 + spellSteal.CastPoint * 1000 + 100,
       ),
      );
     }
    }
   ...
   if (spellName === "rubick_telekinesis") {
    ...
    if (
     ...
     if (hero.Distance2D(bestTarget) <= castRange) {
      ...
      this.sleeper.Sleep(
       GameState.InputLag * 1000 + telekinesis.CastPoint * 1000 + 100,
      );
      return;
     }
    }
    ...
    if (!stolenSpellsExecuted) {
     ...
     if (
      this.executeStolenSpells(stolenSpells, hero, bestTarget, isTargetImmune)
     ) {
      return;
     }
    }
    ...
    if (
     ...
     if (hero.Distance2D(bestTarget) <= castRange) {
      ...
      this.sleeper.Sleep(
       GameState.InputLag * 1000 + fadeBolt.CastPoint * 1000 + 100,
      );
      return;
     }
    }
