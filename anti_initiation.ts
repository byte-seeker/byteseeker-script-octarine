...
const SUPPORTED_SPELLS: SpellConfig[] = [
 ...
 },
 {
  ...
  label: "Wraithfire Blast",
  piercesBkb: false,
  castType: "target",
 },
 {
  name: "witch_doctor_paralyzing_cask",
  label: "Paralyzing Cask",
  piercesBkb: false,
  castType: "target",
 },
 {
...
new (class AntiInitiationUtility {
 ...
 private readonly antiInitRange = this.antiInitiationNode.AddSlider(
  "Trigger Range",
  450,
  200,
  800,
 ...
 private readonly antiInitSuddenOnly = this.antiInitiationNode.AddToggle(
  "Only on Sudden Arrival",
  true,
 );
 private readonly antiInitDebug = this.antiInitiationNode.AddToggle(
  "Draw Debug Overlay",
  true,
 );
 ...
 private PostDataUpdate(delta: number): void {
  ...
  for (const heroEntity of allHeroes) {
   if (
    heroEntity &&
    heroEntity.IsValid &&
    heroEntity.IsEnemy(hero) &&
    !heroEntity.IsIllusion
   ) {
   ...
   for (const config of itemConfigs) {
    ...
    if (item) {
     ...
     if (itemReady) {
      const isEulSelfCast =
       isTargetImmune &&
       (item.Name === "item_cyclone" || item.Name === "item_wind_waker");
      ...
      if (inRange) {
       ...
       candidates.push({
        ...
        priorityValue: prioritizeSpells
         ? config.priority + 100
         : config.priority,
       });
      }
 ...
 private Draw(): void {
  ...
  if (enemies.length === 0) {
   ...
   for (const enemy of enemies) {
    ...
    RendererSDK.Text(
     `Enemy: ${enemy.Name} | Dist: ${dist.toFixed(
      0,
     )} | wasVisible: ${wasVisible} | range: ${this.antiInitRange.value}`,
     new Vector2(50, y),
     Color.White,
    );
    y += 20;
  ...
  if (testEnemy) {
   ...
   RendererSDK.Text(
    "  (No visible enemies - queue empty)",
    new Vector2(50, y),
    Color.LightGray,
   );
   y += 20;
  ...
  for (const config of SUPPORTED_SPELLS) {
   ...
   const info = `Lvl: ${spell.Level} | CD: ${spell.Cooldown.toFixed(
    1,
   )} | Mana: ${hero.Mana.toFixed(0)}/${
    spell.ManaCost
  ...
  for (const config of itemConfigs) {
   ...
   for (const item of itemsFound) {
    const ready =
     ...
     hero.Mana >= item.ManaCost &&
     item.Cooldown <= 0.1;
    const info = `Ready: ${ready} | CD: ${item.Cooldown.toFixed(1)} | Level: ${
     item.Level
    } | Usable: ${item.CanBeUsable}`;
