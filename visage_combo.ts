...
new (class VisageCombo {
 private readonly entry = Menu.AddEntry("Byteseeker").AddNode("Visage Combo");

 // Enable/Disable combo
 private readonly comboEnabled = this.entry.AddToggle(
  "Enable Combo",
  true,
  "Enable/Disable Visage combo script",
 );

 private readonly comboKey = this.entry.AddKeybind(
  "Combo Key",
  "F",
  "Hold to execute Visage combo",
 );
 ...
 private getControllableFamiliars(): npc_dota_visage_familiar[] {
  ...
  for (const unit of EntityManager.GetEntitiesByClass(
   npc_dota_visage_familiar,
  )) {
 ...
 private executeStoneFormChainStun(
  ...
  for (const familiar of familiars) {
   const stoneForm = familiar.GetAbilityByName(
 ...
 private executeAutoSoulAssumption(hero: Hero): void {
  ...
  const castRange =
   soulAssumption.CastRange > 0 ? soulAssumption.CastRange : 900;
  ...
  this.autoSoulSleeper.Sleep(
   GameState.InputLag * 1000 + soulAssumption.CastPoint * 1000 + 100,
  );
 }
  ...
  if (visageHpPct <= this.visageSaveHpPct.value) {
   if (
    ...
    if (
     ...
     if (closestEnemy) {
      ...
      this.autoSaveSleeper.Sleep(
       GameState.InputLag * 1000 + graveChill.CastPoint * 1000 + 100,
      );
      return;
     }
    }
   ...
   if (
    ...
    if (
     ...
     this.autoSaveSleeper.Sleep(
      GameState.InputLag * 1000 + silentGrave.CastPoint * 1000 + 100,
     );
     return;
    }
   }
   ...
   if (
    ...
    const gravekeepersCloak = hero.GetAbilityByName(
    ...
    if (
     ...
     this.autoSaveSleeper.Sleep(
      GameState.InputLag * 1000 + gravekeepersCloak.CastPoint * 1000 + 100,
     );
     return;
    }
   }
 ...
 private PostDataUpdate(delta: number): void {
  ...
  const summonEnabled = this.comboSequenceGrid.IsEnabled(
   "visage_summon_familiars",
  );
