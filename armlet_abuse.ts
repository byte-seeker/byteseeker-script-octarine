...
class SmartArmletAbuse {
 private readonly entry =
  Menu.AddEntry("Byteseeker").AddNode("Smart Armlet Abuse");

 private readonly enabled = this.entry.AddToggle("Enabled", true);
 private readonly toggleKey = this.entry.AddKeybind(
  "Toggle Keybind",
  "None",
 ...
 // Safety Options
 private readonly safetyNode = this.entry.AddNode(
  "Safety Settings",
  "",
  "",
  -1,
 ...
 private readonly projectileBuffer = this.safetyNode.AddSlider(
  "Projectile Buffer Time (ms)",
  250,
  100,
  1000,
 );
 private readonly attackBuffer = this.safetyNode.AddSlider(
  "Attack Animation Buffer Time (ms)",
  350,
  100,
  1000,
 );
 ...
 private PostDataUpdate(delta: number): void {
  ...
  const hasUnholyStrength = hero.Buffs.some(
   (buff) => buff.Name === "modifier_item_armlet_unholy_strength",
  );
 ...
 private isThreatPresent(hero: Hero): boolean {
  ...
  if (this.checkAnimations.value) {
   ...
   for (const enemy of enemies) {
    ...
    if (
     enemy.IsInAnimation &&
     enemy.LastAnimationIsAttack &&
     !enemy.LastAnimationCasted
    ) {
     const remainingTime =
      enemy.LastAnimationStartTime + enemy.LastAnimationCastPoint - now;
 ...
 private Draw(): void {
  ...
  RendererSDK.Text(
   `Armlet: FOUND | Unholy Strength: ${
    hasUnholyStrength ? "ACTIVE" : "INACTIVE"
   }`,
   new Vector2(50, y),
   Color.Green,
  );
  ...
  for (const proj of projectiles) {
   ...
   const bufferTime = this.projectileBuffer.value / 1000;
   if (timeToImpact <= bufferTime) {
    threatInfo = `Projectile (Impact in ${timeToImpact.toFixed(
     2,
