...
new (class PudgeCombo {
 ...
 private readonly orbWalkEnabled = this.entry.AddToggle(
  "Enable Smart Orb Walk",
  true,
 );
 private readonly orbWalkDist = this.entry.AddSlider(
 ...
 private readonly predBufMs = this.hookNode.AddSlider(
  "Latency Buffer (ms)",
  50,
  0,
  200,
 ...
 private readonly dismemberEnabled = this.dismemberNode.AddToggle(
  "Enabled",
  true,
 );
 ...
 private readonly autoHookEnabled = this.autoHookNode.AddToggle(
  "Enabled",
  false,
 );
 private readonly autoHookMinDist = this.autoHookNode.AddSlider(
  "Min Distance",
  200,
  0,
  600,
 ...
 private readonly farmEnabled = this.farmNode.AddToggle(
  "Enabled",
  false,
 ...
 private readonly espRangeCircle = this.espNode.AddToggle(
  "Show Hook Range Circle",
  true,
 );
 private readonly espHookLine = this.espNode.AddToggle(
  "Show Hook Trajectory",
  true,
 );
 private readonly espRangeLabel = this.espNode.AddToggle(
  "Show Range Labels",
  true,
 );
 private readonly espApproachSec = this.espNode.AddSlider(
  "Approach Warning (sec)",
  2,
  1,
  5,
  1,
 ...
 private isDirectionStable(idx: number): boolean {
  ...
  const moving = window.filter(
   (s) => Math.sqrt(s.vx * s.vx + s.vy * s.vy) > 20,
  );
  if (moving.length < 2) {
   return true;
  } // stationary = stable
 ...
 private OnDraw(): void {
  if (!this.hasLocalHero || ExecuteOrder.DisableHumanizer) {
   return;
  }
 ...
 private isVulnerable(target: Hero): boolean {
  return (
   target.IsStunned ||
   target.IsHexed ||
   target.Buffs.some((b: any) => b.Name === "modifier_pudge_meat_hook")
  );
 }
 ...
 private castHook(hero: Hero, target: Hero): boolean {
  ...
  if (this.isVulnerable(target) && this.dismemberEnabled.value) {
   return false;
  }

  // ── 0.3s direction stability check ──
  if (this.requireStable.value && !this.isDirectionStable(target.Index)) {
   return false;
  }
 ...
 private castRot(hero: Hero, target: Hero | undefined): boolean {
  if (!this.rotEnabled.value || this.rotSleeper.Sleeping) {
   return false;
  }
 ...
 private runAutoHook(hero: Hero): void {
  if (!this.autoHookEnabled.value || this.autoHookSleeper.Sleeping) {
   return;
  }
  if (hero.IsChanneling || hero.IsStunned || hero.IsSilenced || hero.IsHexed) {
   return;
  ...
  if (this.requireStable.value && !this.isDirectionStable(best.Index)) {
   return;
  }
  ...
  this.autoHookSleeper.Sleep(
   GameState.InputLag * 1000 + hook.CastPoint * 1000 + 300,
  );
 }
 ...
 private PostDataUpdate(delta: number): void {
  ...
  if (!this.rotSleeper.Sleeping) {
   this.castRot(hero, best);
  }

  if (!best || this.sleeper.Sleeping) {
   return;
  }

  // Priority: dismember when target is already vulnerable (hooked/stunned)
  if (
   this.isVulnerable(best) &&
   this.comboSequenceGrid.IsEnabled("pudge_dismember")
  ) {
   if (this.castDismember(hero, best)) {
    return;
   }
  }
  ...
  for (const name of this.comboSequenceGrid.values) {
   if (!this.comboSequenceGrid.IsEnabled(name)) {
    continue;
   }
   if (name === "pudge_meat_hook" && this.castHook(hero, best)) {
    return;
   }
   if (name === "pudge_rot") {
    continue;
   } // handled separately
   if (name === "pudge_dismember" && this.castDismember(hero, best)) {
    return;
   }
  }
