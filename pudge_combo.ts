...
new (class PudgeCombo {
	...
	private isVulnerable(target: Hero): boolean {
		return target.IsStunned || target.IsHexed || target.Buffs.some(b => b.Name === "modifier_pudge_meat_hook")
	}
	...
	private castRot(hero: Hero, target: Hero | undefined): boolean {
		...
		const shouldOn =
			target !== undefined && hero.Distance2D(target) <= this.rotMaxRange.value && !target.IsMagicImmune
	...
	private PostDataUpdate(delta: number): void {
		...
		const hero = LocalPlayer?.Hero
		if (!hero || !hero.IsValid || !hero.IsAlive || !this.comboEnabled.value || !this.comboSequenceGrid) {
			return
		}
