...
export const PudgeConfig = new (class {
  public readonly entry = Menu.AddEntry("Byteseeker")
    .AddNode("Hero")
    .AddNode("Strength")
    .AddNode("Pudge");
  ...
  public readonly predBufMs = this.hookNode.AddSlider("Latency Buffer (ms)", 50, 0, 200, 0, "Added on top of physics intercept for server latency");
  public readonly requireStable = this.hookNode.AddToggle("Require Direction Stability", true, "Observe 0.3s enemy movement before firing hook");
  public readonly autoKsEnabled = this.hookNode.AddToggle("Auto KS with Hook", true, "Automatically hook enemies if their HP is low enough to die");
  public readonly stableThreshDeg = this.hookNode.AddSlider("Stability Threshold (deg)", 25, 5, 60, 0, "Max direction deviation to consider movement stable");
  ...
  public readonly farmSafeHpPct = this.farmNode.AddSlider("Safe HP %", 40, 10, 80, 0, "Turn off Rot if Pudge HP drops below this %");
  public readonly farmCreepHpPct = this.farmNode.AddSlider("Creep HP Threshold %", 10, 1, 100, 0, "Minimum creep HP % to activate Rot");
  public readonly farmMoveToWave = this.farmNode.AddToggle("Move to Nearest Creep", true, "Walk toward nearest creep if none in Rot range");
